...
<ParamField body="event" type="enum" default="clicks">
  ...
  `leads`, `sales` and `composite`.
</ParamField>

<ParamField body="groupBy" type="enum">
  The parameter to group the analytics data points by. Defaults to 'count' if
  undefined. Available options: `count`, `timeseries`, `continents`, `regions`,
  `countries`, `cities`, `devices`, `browsers`, `os`, `trigger`, `triggers`,
  `referers`, `referer_urls`, `top_links`, `top_urls`.
</ParamField>
