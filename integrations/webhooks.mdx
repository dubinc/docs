...
- Extract the signature from the `Dub-Signature` header.
- Calculate the HMAC of the raw body using the `SHA-256` hash function and the secret.
- Compare the calculated `HMAC` with the one sent in the `Dub-Signature` header. If they match, the webhook is verified.
