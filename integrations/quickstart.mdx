...
<Steps>
  ...
  <Step title="Refresh the access token">
    Dub access tokens are short-lived, depending on the `expires_in` value (the default value is `7200 seconds`, or **2 hours**). Dub will respond with `401 Unauthorized` if you try to use an expired access token.
