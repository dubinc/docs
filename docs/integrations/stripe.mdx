...
<Note>
  ...
  generated. [Sale
  rewards](/help/article/partner-rewards#configuring-reward-types) (both recurring and one time rewards) will still be
  generated as usual.
</Note>
...
<Tip>
  ...
  reward for everything else, an invoice with `$100` of `prod_gold` and `$50`
  of `prod_addon` will generate a commission of `($100 × 40%) + ($50 × 10%) =
  $45`.
</Tip>
