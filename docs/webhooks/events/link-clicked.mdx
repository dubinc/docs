...
<Tip>
  Due to the high volume nature of these events, you can scope which links trigger a `link.clicked` webhook. When creating or editing the webhook in the dashboard, choose one of the following:
  ...
  * **Include specific links** – trigger the webhook only for the links you explicitly select.
  ...
  You can also associate specific links with a `link.clicked` webhook programmatically using the [`webhookIds` prop in the Links API](/docs/api-reference/links/create#body-webhook-ids-one-of-0).
</Tip>
