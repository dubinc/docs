...
<Warning>
  If your app uses other SDKs that handle URLs (e.g., Facebook/Meta SDK, Google Sign-In), make sure your `AppDelegate`'s `openURL` handler doesn't short-circuit with `||`. When a third-party SDK returns `true` first, Dub's `handleDubDeepLink` will never be called. See the [troubleshooting guide](/docs/concepts/deep-links/quickstart#troubleshooting-deep-links) for details and a fix.
</Warning>
