...
If you're using a non-React framework, you need to insert the script into the header section of your site.

You can do this by adding the following Dub analytics script within the `<head>` … `</head>` tags.

```html
<script src="https://www.dubcdn.com/analytics/script.js" defer></script>
```

Depending on the platform you're using, you can install the script in different ways:
