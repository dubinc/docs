...
<Steps>
  ...
  <Step title="Add Sales Tracking Tag">
    ...
    4. Configure the tag settings:
       ...
       > You can use GTM variables to dynamically map values from the incoming event data objects to the fields in the Dub Conversion Tag. For example, you might use a variable like `{{Event Data - customerExternalId}}` to populate the Customer External ID field.
