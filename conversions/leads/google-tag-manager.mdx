...
<Steps>
  ...
  <Step title="Create Dub Server Client">
    ...
    Create a new Client using the imported template:
  ...
  <Step title="Import Dub Conversion Tag Template">
    ...
    Import the Dub Conversion Tag template to handle lead tracking:
  ...
  <Step title="Add Lead Tracking Tag">
    ...
    Create a new tag for lead tracking with proper mapping:
...
The Dub GTM Server Client template works by:

1. **Receiving requests** at the configured path (`/dub/track`)
2. **Parsing query parameters** from the incoming request
3. **Structuring the data** into a standardized event format
4. **Processing the event** through your GTM Server container
5. **Sending to destinations** based on your tag configuration
...
<Card
  ...
  icon="github"
  href="https://github.com/dubinc/dub-gtm-server-client"
>
  Dub GTM Server Client template
</Card>
