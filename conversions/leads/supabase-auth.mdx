...
1. In the `/api/auth/callback` route, check if:
   - the `dclid` cookie is present.
   - the user is a new sign up (created in the last 10 minutes).
2. If the `dclid` cookie is present and the user is a new sign up, send a lead event to Dub using `dub.track.lead`
3. Delete the `dclid` cookie.
  ...
  if (typeof code === "string") {
    ...
    if (!error) {
      ...
      if (dclid && isNewUser) {
        ...
        res.setHeader(
          "Set-Cookie",
          `dclid=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT`
        );
      }
