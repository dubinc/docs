...
1. **Rigid payouts**: Payouts only run on Lemon Squeezy's own fixed schedule (created on the 1st/15th, paid on the 14th/28th), with a mandatory 30-day hold on every commission, so you cannot set your own payout schedule.
2. **Only per-sale commissions**: You can only set a flat-rate or percentage-based commission per sale (per affiliate or per product) – there's no support for per-click or per-lead rewards, conditional rewards, or bounties.
3. **No affiliate groups or tiers**: Every affiliate is treated the same way – there's no way to segment partners into groups or tiers with different terms.
...
1. **Automated payouts:** You can set your own payout schedule and [pay your partners globally](/help/article/partner-payouts) in 1-click, with tax compliance built-in. And Dub’s automated [risk detection](https://dub.co/help/article/risk-monitoring) helps ensure that only legitimate affiliate activity is rewarded.
...
3. **Partner groups:** You can organize partners into [groups](https://dub.co/help/article/partner-groups) based on rewards, performance, location, and more, making it easier to tailor incentives and track results. You can also use [tags](https://dub.co/help/article/partner-tags) to classify partners by type, campaign, outreach status, or any other criteria.
4. **White-labeling**: You can [embed a white-labeled referral dashboard into your application](/docs/partners/embedded-referrals) for a better user experience.
...
<Steps>
  ...
  <Step title="Import your Lemon Squeezy store in the Partners tab">
    ...
    To save you some time, [here's a quick link](https://app.dub.co/program/partners?import=lemonsqueezy) that will get you directly to the Lemon Squeezy import flow.
...
We only import affiliates with a `status` of `active` on Lemon Squeezy – pending or disabled affiliates are skipped, along with any affiliate that doesn't have an email on file. Since Lemon Squeezy doesn't have affiliate groups or campaigns, all imported affiliates land in your program's default [partner group](/help/article/partner-groups).

Each imported partner gets a Dub partner link, where the link's key is set to their Lemon Squeezy affiliate ID, ensuring future sales are attributed correctly. We also import each partner's `user_email` and `user_name`.

### Customers

We only import customers who are associated with an affiliate in Lemon Squeezy; customers without an affiliate are excluded. Each customer's `externalId` is set to their Lemon Squeezy customer ID, and the customer is attributed to the first associated affiliate for whom a partner link was successfully imported.
...
| `void`, `failed` | `Canceled` |
| `fraudulent` | `Fraud` |

We use `subtotal_usd` for the sale amount where available, falling back to a currency conversion of `subtotal` otherwise.

Additionally, Dub also supports 2 extra commission states that aren't set by the importer, but that you can use going forward:
