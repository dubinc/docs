...
<Warning>
  Lemon Squeezy doesn't expose a per-order commission amount for affiliates.
  Because of this, **earnings for imported commissions are calculated using your
  Dub program's configured sale reward** (not your historical Lemon Squeezy
  commission rates). If you had custom commission rates set per affiliate or per
  product on Lemon Squeezy, those are not carried over – make sure your Dub
  program's [reward](/help/article/partner-rewards) is configured before
  importing.
</Warning>
