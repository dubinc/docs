...
<Info>
  **Example:** Move partners to the `VIP Group` once their total commissions reach $100, but only if they're currently in the `Affiliate` or `Creator` groups.
</Info>

You can also add an optional group condition to limit the rule based on a partner's current group. 
