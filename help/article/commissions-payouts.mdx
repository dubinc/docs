...
Note that the holding period only applies to commissions from `click`, `lead`, and `sale` events. Commissions from [bounties](/help/article/program-bounties) or one-time custom commissions from the program are **not** subject to the holding period, and become eligible for payout at the next payout cycle.
