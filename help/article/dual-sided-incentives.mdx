...
By default, the discount is configured at the [partner group](/help/article/partner-groups) level. You can also override it for an individual [partner](#partner-level-discounts) or [link](#link-level-discounts).

In this article, we'll learn how to set up dual-sided incentives with Dub.

## Stripe coupons / promotion codes

There are two ways to set up dual-sided incentives if you're using Stripe:
...
The key to implementing link-based discounts is to check if a customer is eligible for a discount using Dub's [Customers API](/docs/api-reference/customers/list), then automatically apply the appropriate Stripe coupon to their checkout session.

Here's how the flow works:

1. When a user clicks a partner referral link, Dub tracks them as a potential customer
2. During checkout, you query the Dub [Customers API](/docs/api-reference/customers/list) using the user's ID
...
<Note>
  This requires the [Dub Shopify app](/docs/integrations/shopify). Dub creates
  and deletes discount codes in your Shopify store automatically.
</Note>

<Steps>
<Step title="Connect the Dub Shopify app">
Install the [Dub Shopify App](https://d.to/shopify/app) and connect it to your Dub workspace. Follow the [Shopify integration guide](/docs/integrations/shopify) if you haven't already.

<Frame>
  <img src="/images/shopify/shopify-app.png" alt="The Dub Shopify app" />
</Frame>

The app needs permission to create discount codes (`write_discounts`). If you installed it before this permission was added, reinstall or upgrade the app from the [Shopify App Store](https://apps.shopify.com/dub-conversion-tracking).

</Step>

<Step title="Create a Shopify discount on Dub">
First, navigate to the partner group that you want to create a discount for. Under the Discount tab, you'll be able to create a discount for the group.
...
<Frame>
  <img
    src="/images/dub-partners/create-shopify-discount.png"
    alt="Program discounts tab"
  />
</Frame>
...
Deleting a discount or discount code on Dub also disables the corresponding codes in Shopify.

</Warning>
</Step>

<Step title="Create partner discount codes">
Open the partner profile that you'd like to create a discount code for, then click **Create Code** in the Discount codes section.
...
<Steps>
<Step title="Create a custom discount on Dub">
First, navigate to the partner group that you want to create a discount for. Under the Discount tab, you'll be able to create a discount for the group.
...
<Frame>
  <img
    src="/images/dub-partners/create-custom-discount.png"
    alt="Program discounts tab"
  />
</Frame>
...
<Tip>
  You can also [create](/docs/api-reference/discount-codes/create) or
  [delete](/docs/api-reference/discount-codes/delete) discount codes via the
  API.
</Tip>
...
<Step title="Sync codes with your system">
Create a [webhook](/docs/webhooks/introduction) subscribed to [`discount_code.created`](/docs/webhooks/events/discount-code-created) and [`discount_code.deleted`](/docs/webhooks/events/discount-code-deleted).

Your endpoint should:

1. [Verify the webhook signature](/docs/webhooks/verify-webhook-requests)
...
These events also fire when codes are [auto-provisioned](#auto-provision-discount-codes), and when a code is removed automatically (for example if a partner is banned, deactivated, or moved to a group without an equivalent discount).
...
export const POST = async (req: Request) => {
  ...
  if (payload.event === "discount_code.created") {
    // Create the promo code in your system
    // payload.data.code, payload.data.discount
  }

  if (payload.event === "discount_code.deleted") {
    // Disable the promo code in your system
    // payload.data.code
  }
...
<Tip>
  Partner-level and link-level discounts are only available on [Advanced plans
  and above](https://dub.co/pricing).
</Tip>

By default, every partner uses the discount configured for their [partner group](/help/article/partner-groups). If you want to make an exception without creating a whole new group, you can override the discount for a single partner.

Partner-level discounts apply to one partner across all of their links, including links they create later, unless a link has its own [link-level override](#link-level-discounts).

Open the partner's details page from **All Partners**. The **Rewards** card on the right shows the discount currently in effect for that partner.
...
Before saving, you'll be asked to confirm the change and can add an optional note to the partner.

If the partner has [Stripe discount codes](#option-2-using-stripe-promo-codes-no-code-required), Dub automatically remaps their discount codes to the new discount so the customer-facing coupon matches what the partner advertises.

<Note>
  Partners with a partner-level discount are automatically excluded from any [group move rules](/help/article/partner-groups#group-move-rules), so they won't be accidentally moved to another group and lose their custom discount.
</Note>

To override a reward for an individual partner instead, see [partner-level rewards](/help/article/partner-rewards#partner-level-rewards).

## Link-level discounts

Link-level discounts apply to one link only. They're useful when the same partner runs several campaigns with different customer offers, for example:
...
<Tip>
  You can also set a link-level discount when you [create a link for a
  partner](/help/article/managing-program-partners#links). The **New partner
  link** modal has a discount selector.
</Tip>

If the partner has [Stripe discount codes](#option-2-using-stripe-promo-codes-no-code-required), Dub automatically remaps their discount codes to the new discount so the customer-facing coupon matches what the link advertises.

<Note>
  Partners with a link-level discount are automatically excluded from any [group move rules](/help/article/partner-groups#group-move-rules), so they won't be accidentally moved to another group and lose their custom discount.
</Note>

To override a reward for a single link instead, see [link-level rewards](/help/article/partner-rewards#link-level-rewards).
...
<Frame>
  <img
    ...
    alt="Partner discount indicator"
  />
</Frame>
