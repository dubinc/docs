...
- A [submitted lead](/help/article/submitted-leads) is marked as `Closed won`

Each sale event creates a [commission](/help/article/partner-commissions) for the partner whose link the customer came from. The commission amount is determined by the sale reward configured for that partner's [group](/help/article/partner-groups), plus any [conditions](#sale-reward-conditions) that match the sale.
...
<Frame>
  <img
    src="/images/dub-partners/create-reward-sale.png"
    alt="Group rewards"
  />
</Frame>

When the reward builder open, set your amount per sale, and optionally set your [sale reward conditions](#sale-reward-conditions).
...
| **Source**  | How the customer was created. See [sale sources](#sale-sources).                       |
...
| --------- | ------------ |
| `tracked sale`    | The sale was tracked via the [`/track/sale`](/docs/api-reference/track/sale) endpoint, an SDK, or the [Stripe integration](/docs/integrations/stripe).     |
...
When you change a group's sale reward, historical commissions aren't changed, but the new configuration applies to all future sales, including renewals from customers the partner referred before the change. If you switch a group from one-time to recurring, previously referred customers' renewals become eligible for commission again.
