...
<Tip>
  **Multiple line items in a single invoice:** If you're using our [Stripe
  integration](/docs/integrations/stripe) and an invoice contains multiple line
  items (e.g. a base subscription plus an add-on), Dub will evaluate each line
  item's product ID against your product-specific reward modifiers and
  calculate the commission for each line item using the matching reward
  rate. The final commission for the invoice is the sum of the per-line-item
  commissions.

For example, if a partner has a 40% reward for `prod_gold` and a 10% reward
for everything else, an invoice with $100 of `prod_gold` and $50 of
