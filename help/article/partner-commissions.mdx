...
<Steps>
  <Step title="Commissions table">
    From the Commissions table, click the `⋯` button and select **Edit
    commission**.
    <Frame>
      <img
  ...
  <Step title="Single commission page">
    From the [single commission
    page](/help/article/partner-commissions#single-commission-view), click the
...
<Note>
  One-time commissions are recorded as `custom` commissions, which are **not**
  subject to your program's [payout holding
  period](/help/article/partner-payouts#payout-holding-period). They become
  eligible for payout immediately at the next payout cycle. The same applies to
  [bounty commissions](/help/article/program-bounties) and `custom` commissions
  created via the [API](/docs/api-reference/commissions/create).
</Note>
...
<Tip>
  To export commissions with any filters you currently have applied, make sure
  to toggle on "Apply current filters". This will ensure that only the
  commissions matching your current filters will be included in the export.
</Tip>
