...
<Accordion title="Reconcile payouts regularly">
  Periodically compare payouts in your system with Dub's records to catch any
  discrepancies. You can use the [Commissions
  API](/api-reference/endpoint/retrieve-a-list-of-commissions) to fetch payout
  data.
</Accordion>
...
<Accordion title="Webhook not receiving events">
  1. Verify your webhook is subscribed to the `payout.confirmed` event 2. Check
  that your webhook URL is publicly accessible 3. Ensure your endpoint returns a
  2xx status code 4. Review webhook logs in your [webhook
  settings](https://app.dub.co/settings/webhooks)
</Accordion>

<Accordion title="Partner not receiving external payouts">
  1. Verify the partner has a `tenantId` configured 2. Check that your program's
  payout mode is set to Hybrid or External 3. Ensure the partner meets the
  minimum payout threshold 4. Confirm the partner's status is "approved"
</Accordion>
