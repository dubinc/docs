...
When a user clicks a link without the app, they're redirected to the app store. After installing and opening the app, you can retrieve the original link information and redirect them to the appropriate screen.
...
- Dub redirects the user to the App Store or Play Store by using [device targeting](/help/article/device-targeting)
- User installs your app from the app store
- App reads the install referrer on first launch
- App track the open using the [`/track/open`](/concepts/links/deep-links/quickstart#track-the-deep-link-open) endpoint
