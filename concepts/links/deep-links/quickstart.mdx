...
func handleDeepLink(url: URL) {
    // Extract domain and key from the URL
    let domain = url.host ?? ""
    let key = url.lastPathComponent

    // Call the tracking endpoint
    trackDeepLinkClick(domain: domain, key: key)
}
```
...
Once you’ve detected and parsed the deep link, you should track the open event and retrieve the final destination URL.
...
Now you've got the destination URL, you can navigate to it in your app.

<CodeGroup>

```swift DeepLinkTracker.swift
func trackDeepLinkClick(domain: String, key: String) {
    ...
    URLSession.shared.dataTask(with: request) { data, response, error in
        if let data = data,
           ...
           let destinationUrl = json["url"] as? String {
...
```javascript DeepLinkTracker.js
async function trackDeepLinkClick(domain, key) {
  try {
...
When your app isn't installed, the user will be redirected to the App Store or Google Play Store. After they install and open your app, you'll need to use [deferred deep linking](/concepts/links/deep-links/deferred-deep-linking) to retrieve the original destination URL.

<Steps>

<Step title="Check for deferred deep link">

When your app opens for the first time after installation, check if the user came from a deep link:
...
func checkForDeferredDeepLink() {
    // Use a deferred deep linking service or check Google Play Store Install Referrer
    // See the deferred deep linking guide for implementation details
}
```
...
override fun onCreate(savedInstanceState: Bundle?) {
    ...
    // Check for deferred deep link using Install Referrer API
    checkForDeferredDeepLink()
}

private fun checkForDeferredDeepLink() {
    // Implement deferred deep linking logic
    // See the deferred deep linking guide for implementation details
}
```
...
<Step title="Follow Scenario 1 flow">

Once you retrieve the original deep link URL through deferred deep linking, follow the same process as Scenario 1:

1. Extract the domain and key from the URL
2. Call the `/track/open` endpoint
3. Navigate to the destination using the returned URL
...
<Note>
  ...
  Deep Linking](/concepts/links/deep-links/deferred-deep-linking) guide.
</Note>

### Troubleshooting

- **App not opening from deep link**: Verify your deep link configuration files are properly set up and accessible
- **Tracking endpoint errors**: Check that your domain and key are correctly extracted from the URL
- **Navigation issues**: Ensure your app can handle the destination URL format and parameters
- **Deferred deep linking not working**: Verify your deferred deep linking implementation and check the [deferred deep linking guide](/concepts/links/deep-links/deferred-deep-linking) for common issues
