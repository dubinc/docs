...
1. **Probabilistic clipboard-based tracking**: Matches users from the web to the app based on their IP address.
2. **Deterministic IP-based tracking**: Uses iOS clipboard data to reliably identify and open the exact deep link the user interacted with.

For the deterministic approach, when an iOS user taps a deep link without your app installed, Dub redirects them to a custom landing page before the App Store.
...
#### 1. Probabilistic clipboard-based tracking
...
#### 2. Deterministic IP-based tracking
