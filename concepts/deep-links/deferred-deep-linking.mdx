...
Add the `trackOpen()` call in your SwiftUI app’s root view `.onAppear`, as shown above.

This ensures tracking happens as soon as the app launches for the first time after installation, when clipboard and IP-based matching are still valid.
  ...
  var body: some View {
    NavigationStack {
      VStack {
        ...
        NavigationLink(
          destination: DetailView(url: deepLinkURL ?? ""),
          isActive: $navigate
        ) {
          EmptyView()
        }
...
1. **First Launch Detection**: Both tracking methods should only run on the first app launch after installation to avoid duplicate tracking.
2. **Network Operations**: All API calls are made on background threads to ensure smooth app performance.
