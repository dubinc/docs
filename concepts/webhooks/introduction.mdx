...
- Sending click events in real-time to [Segment](https://app.dub.co/integrations/segment) for further processing
- Get a [Slack](https://app.dub.co/integrations/slack) notification when someone clicks on your pitch deck link
...
This event is triggered when a [new sale is created](/api-reference/endpoint/track-sale) via [Dub Conversions](/conversions/quickstart). The event payload contains the following:
